def signal(df, n, factor_name, config):
    ...
    df["mtm"] = (df["close"] / (df["ma"] + config.eps).shift(n) - 1) * 100
    ...
    df["ATR"] = df["ATR_abs"] / (df["ma"] + config.eps) * 100
    ...
    df["taker_sell_ma"] = (df["taker_sell_quote_asset_volume"] / (df["vma"] + config.eps)) * 100
