def signal(df, n, factor_name, config):
    ...
    df["close_change"] = (df["close"] / (df["close"].shift(n) + config.eps) - 1).ewm(span=n, adjust=config.ewm_adjust).mean() * 100
    df["vol_change"] = (df["quote_volume"] / (df["quote_volume"].shift(n) + config.eps) - 1).ewm(
