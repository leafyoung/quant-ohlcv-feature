def signal(df, n, factor_name, config):
    ...
    ema_ema_1 = ema1.ewm(span=n, adjust=config.ewm_adjust).mean()
    n2 = 5 * n
    ema2 = df["close"].ewm(span=n2, adjust=config.ewm_adjust).mean()
    ema_ema_2 = ema2.ewm(span=n2, adjust=config.ewm_adjust).mean()
