...
def signal(df, n, factor_name, config):
    ...
    df = df.with_columns(pl.Series("c_mtm", df["close"] / df["close"].shift(n) - 1))
    ...
    df = df.with_columns(pl.Series("s_mtm", df["std"] / df["std"].shift(n)))
    df = df.with_columns(pl.Series("s_mtm", df["s_mtm"].rolling_mean(n, min_samples=config.min_periods)))
    # volume change
    df = df.with_columns(pl.Series("v_mtm", df["quote_volume"] / df["quote_volume"].shift(n)))
