...
def signal(df, n, factor_name, config):
    ...
    df = df.with_columns(pl.Series("Vi+", df["sum_pos"] / (df["sum_tr"] + config.eps)))
    df = df.with_columns(pl.Series("Vi-", df["sum_neg"] / (df["sum_tr"] + config.eps)))
    df = df.with_columns(pl.Series(factor_name, df["Vi+"] - df["Vi-"]))
