...
def signal(df, n, factor_name, config):
    ...
    # Formula: REG = LINEARREG(CLOSE, 2N); result = 100 * (CLOSE - REG) / (REG + config.eps)
    ...
    df = df.with_columns(pl.Series(factor_name, 100 * (df["close"] - df["LINEARREG"]) / (df["LINEARREG"] + config.eps)))
