...
def signal(df, n, factor_name, config):
    ...
    ema1 = (0.5 * df["high"] + 0.5 * df["low"]).ewm_mean(span=n, adjust=config.ewm_adjust)
    ema2 = (0.5 * df["high"] + 0.5 * df["low"]).ewm_mean(span=2 * n, adjust=config.ewm_adjust)

    dif = ema1 - ema2
    dea = dif.ewm_mean(span=int(n / 2.0), adjust=config.ewm_adjust)

    df = df.with_columns(pl.Series(factor_name, 10 * (2 * dif - dea)))
