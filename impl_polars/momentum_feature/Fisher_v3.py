...
def signal(df, n, factor_name, config):
    ...
    df = df.with_columns(pl.Series("price_change", 0.3 * df["price_change"] + 0.7 * df["price_change"].shift(1)))
    df = df.with_columns(
        pl.Series(
            factor_name,
            0.5 * df["price_change"].shift(1) + 0.5 * np.log((1 + df["price_change"]) / (1 - df["price_change"])),
        )
    )
    ...
    # fisher = 0.5 * pl.Series(price_ch).shift(1) + 0.5 * pl.Series(np.log((1 + price_ch) / (1 - price_ch)))
    #
    # signal = fisher
